...
struct ResPage //0x10
{
	uint32_t m_unused;								//0x0	always 0 ?
	uint32_t m_pageSize;							//0x8	size of the whole page incl the header
	uint16_t m_unk;									//0x08	owning page idx ?
	uint16_t m_unused2;								//0x0A	0xDEAD 
	uint16_t m_unused3;								//0x0C	0xBEEF
	uint16_t m_numPageHeaderEntries;				//0x0E	number of pointers in the table
};
